<template>
  <div>
    ...
    <div class="selected-thumbs">
        ...
        <div v-if="selectedImage"  class="thumb-wrapper">

          <v-img :src="getLink(selectedImage)" width="40" height="40" @click="openPreview(selectedImage)"></v-img>

          <v-btn icon small class="remove-btn" @click.stop="removeImage()">
            ...
            <v-row>
              <div v-for="image in availableImages" :key="image.id" cols="3">
                <v-card
                  ...
                  :class="{ 'selected-image': selectedImage.id == image.id }"
...
export default {
  ...
  data() {
    return {
      ...
      previewModal: false,
      previewImage: {},
      selectedImage: [], // Gemmer hele billedobjekter internt
      availableImages: [],
      loading: false,
      isUploading: false,
      totalFiles: 0,
  ...
  watch: {
    // Når selectedImage ændres, emitteres kun ID'erne
    selectedImage: {
      handler() {
        this.returnImages();
      },
  ...
  methods: {
    ...
    fetchImages() {
      ...
      axios
        .post(route("api.media.media.search"), {
  ...
  mounted() {
    ...
    // Her sætter vi selectedImage til en tom liste, men du kan implementere en logik, der matcher ID'er med objekter.
